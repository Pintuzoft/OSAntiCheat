...
**Live as of [v0.9.8](https://github.com/Pintuzoft/OSAntiCheat/releases):** all eight Tier-1 axes
...
something the population has never produced. Four Tier-1 signatures additionally carry an
auto-action edge, one of them latent (see [Enforcement](#enforcement)); everything else is log + admin notice.
...
| **Silent aim** | A bullet *registers damage* while the view points ≥10° away from every position the victim held in the lag-comp window; since v0.9.113 a *head* hit with a precision weapon (pistols, snipers) at ≥700u counts from 4°, and two distinct victims on one map is the `silent-far-hs` edge | Honest burst-opener max: 8.0° (0 of 3,486 ≥10°). Far precision headshots: deagle p99 1.94°, >4° in 0.07% (n=5,362); two per session in 0 of 42,342 honest archive sessions. Catches spin-silent, frozen-view psilent and the 5° "two metres beside" variant (C5, C6, C11) |
...
| **Aim drift** (v0.9.94) | Share of aim *steps* that reduce the error to an enemy, against the lobby's own share (two-proportion z) — the soft-aim pull | Corroborate-only since v0.9.108: its honest tail is population-wide (55 players in one week), so it may double a case another axis opened, never build one |
...
- **`airgain-chain`** (v0.9.101) — ≥5 chained jump arcs with median mid-air gain ≥ +40 u/s at
  ...
  ticket" announce) — a neutralised cheat that stays on the server is preserved evidence.
  **Latent by default** (`AirGainFreezeArmed=false`): the edge fires, fuses, alerts admins red
  and logs the dry-run, but nobody freezes until the first real catch reads clean.
