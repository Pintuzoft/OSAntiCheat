...
if (args.Length < 1)
{
    Console.Error.WriteLine("usage: osac-replay <demo.dem | directory> [--poll 0.05] [--csv out.csv] [--shots out.csv] [--kills out.csv] [--hurts out.csv]");
    return 1;
}
...
StreamWriter? hurtsCsv = null;
if (hurtsPath is not null)
{
    // The silent-aim raw measurement, one row per enemy bullet hurt (pellets excluded): the
    // population distribution the off-floor gate is read from. burstOpener separates what the
    // detector judges (recoil-free first bullets) from the recoil-confounded mid-spray mass.
    bool freshHurts = !File.Exists(hurtsPath) || new FileInfo(hurtsPath).Length == 0;
    hurtsCsv = new StreamWriter(hurtsPath, append: true) { AutoFlush = false };
    if (freshHurts) hurtsCsv.WriteLine("demo,attackerId,attackerName,victimId,round,tick,weapon,dmg," +
        "distU,candidates,minErrDeg,burstOpener");
    hurtsCsv.Flush();
}
    ...
    async (file, _) =>
    {
        try
        {
            ...
            lock (gate)
            {
                ...
                // aimbot.snap: per-player pulls-to-head (off@N-1 -> exact@N) over the demo's shots.
                foreach (var g in shotRows.Where(s => s.SnapErrFire >= 0f).GroupBy(s => s.SteamId))
                {
                    var pulls = g.Where(s => s.SnapErrFire <= cfg.SnapExactDeg && s.SnapErrPrev >= cfg.SnapOffFloorDeg).ToList();
                    if (pulls.Count >= cfg.SnapMinSnaps)
                        snapHits.Add((g.First().Name, g.Key, pulls.Count, g.Count(),
                            pulls.Max(s => s.SnapErrPrev), pulls.Min(s => s.SnapErrFire), Path.GetFileName(file)));
                }
                // aimbot.silent: signal count per player (the detector gates internally: first-of-burst
                // enemy hits with the view >=15° off every lag-window victim position, >=3 repeats).
                foreach (var r in results)
                    if (r.Signals.GetValueOrDefault("aimbot.silent") is > 0 and var sn)
                        silentAlerts.Add((r.Name, r.SteamId, sn, Path.GetFileName(file)));
                foreach (var r in results)
...
if (recoilResults.Count > 0)
{
    ...
    Console.WriteLine($"  knobs: deadaim>={cfg.DeadaimMin:F3}  boneLockSpikes>={cfg.BoneLockMinSpikes}  " +
                      ...
                      $"snap: off>={cfg.SnapOffFloorDeg:F0}°->exact<={cfg.SnapExactDeg:F3}° (>={cfg.SnapMinSnaps})");
    ...
    foreach (var s in snapHits.OrderByDescending(s => s.snaps))
    { t1++; Console.WriteLine($"  [snap]        {s.snaps} pull(s) {s.bestPrev:F1}°off->{s.bestFire:F3}°head over {s.shots} shots  {s.name,-20} {s.id}  [{s.demo}]"); }
    foreach (var s in silentAlerts.OrderByDescending(s => s.signals))
...
static async Task<(List<PlayerResult> players, List<ShotRow> shots, List<KillRow> kills, List<HurtRow> hurts, List<EncounterRow> encounters, List<RotationRow> rotations)> ReplayOne(string file, float pollInterval, ulong revisitTarget, bool printBlocks = true)
{
    ...
    var spinHits = new Dictionary<int, int>();          // shots on-body (<=2°) while spinning (>1000°/s)
    var spinHsKills = new Dictionary<int, int>();        // HEADSHOT kills landed while spinning — the sharpest spinbot tell
    ...
    demo.Source1GameEvents.PlayerHurt += e =>
    {
        ...
        // "flashbang" does not contain "grenade" — it slipped this filter for six archives, counting
        // flash pops as bullet hits (inflated hit% slightly) and owning the entire false honest tail
        // of the silent-aim measurement until the 21-demo hurts sweep exposed it.
        if (w.Contains("grenade") || w.Contains("molotov") || w.Contains("inferno") || w.Contains("flash") ||
        ...
        // aimbot.silent: a bullet REGISTERED on an enemy while the replicated view pointed away from
        // every position the victim held in the lag-comp window. Enemy hits only (team damage is
        // griefing, not aim), first-of-burst only (recoil comp parks the view off-victim mid-spray).
        if (vs2 >= 0 && att.Team != e.Player.Team &&
            ...
            trackers.TryGetValue(s2, out var silentAtt) && trackers.TryGetValue(vs2, out var silentVict))
        {
            ...
            if (!SilentAimDetector.IsPelletWeapon(w))
            {
                float minErr = SilentAimDetector.MeasureMinError(silentAtt, silentVict, 16, out int cand, out float hd);
                if (minErr >= 0f)
                {
                    bool opener = !(fireTimes.TryGetValue(s2, out var hft) && hft.Count >= 2 &&
                                    hft[^1] - hft[^2] < BurstWindowSeconds);
                    hurtRows.Add(new HurtRow(att.SteamID, att.PlayerName ?? "?", e.Player.SteamID,
                        roundNumber, demo.CurrentDemoTick.Value, w, e.DmgHealth, hd, cand, minErr, opener ? 1 : 0));
                }
            }
        }
    };
    ...
    demo.Source1GameEvents.PlayerDeath += e =>
    {
        ...
        // a full turn to aim (the FP that flagged three legit regulars off flicks; all had maxYaw==0).
        ...
        // aimbot.snap approach profile: head-centre aim error at the shot tick (N) and the two ticks
        // before it (N-1, N-2), each to the victim's head at THAT tick. The snap signature is a step —
        // off the head at N-1, exact at N. Exporting N-2 too lets the archive settle whether N-1 alone
        // suffices or a smoothed ramp needs a wider look (expected: bone-lock owns the smoothed case).
        // -1 = not computable (missing/gap/dead sample, or degenerate range).
        float HeadErrAt(int ago)
        {
            if (ago >= aTr.Count) return -1f;
            var a = aTr[ago];
            if (!a.Alive || !vTr.TryGetBySequence(a.Sequence, out var vs) || !vs.Alive) return -1f;
            if (ago > 0 && (aTr[ago - 1].Sequence - a.Sequence != 1)) return -1f; // gap breaks the tick-to-tick claim
            if (Vector3.Distance(a.Origin, vs.Origin) < 64f) return -1f;          // degenerate range
            return Geometry.AimErrorTo(a.Origin + new Vector3(0f, 0f, 64f), a.Angles,
                vs.Origin + new Vector3(0f, 0f, 64f));
        }
    ...
    demo.Source1GameEvents.WeaponFire += e =>
    {
        ...
        ftl.Add(Now());
        silent.NoteFire(slot, Now());   // burst bookkeeping for aimbot.silent's recoil exclusion
        ...
        // snap runs on EVERY shot, mid-burst included — the spray-aimbot pulls only one shot in the
        // spray onto the head, so it lives where the burst-gated detectors never look.
        Report(snap, snap.OnFire(shooterTracker, enemies, now));
        ...
        // aimbot.snap raw profile: head-centre error at this shot (N) and one tick before (N-1), to the
        // SAME target's head at each tick. off@N-1 -> exact@N = a pull-to-head this tick (mid-burst too).
        // No LOS gate: a pull onto a head behind a wall/object lands exact and never becomes a kill, but
        // still shows here. -1 = not computable (gap, dead, or no on-target enemy).
        float snapErrFire = -1f, snapErrPrev = -1f;
        if (nearestErr <= 5f && shooterTracker.Count >= 2 && trackers.TryGetValue(nearestId, out var snapTgt))
        {
            var pv = shooterTracker[1];
            if (atFire.Sequence - pv.Sequence == 1 && pv.Alive &&
                snapTgt.TryGetBySequence(atFire.Sequence, out var st0) && st0.Alive &&
                snapTgt.TryGetBySequence(pv.Sequence, out var st1) && st1.Alive)
            {
                snapErrFire = Geometry.AimErrorTo(eye, atFire.Angles, st0.Origin + new Vector3(0f, 0f, 64f));
                snapErrPrev = Geometry.AimErrorTo(pv.Origin + new Vector3(0f, 0f, 64f), pv.Angles,
                    st1.Origin + new Vector3(0f, 0f, 64f));
            }
        }
...
internal sealed record SweepConfig
{
    ...
    public float SnapExactDeg { get; init; } = 0.05f;       // head-centre error AT the shot = machine precision
    public float SnapOffFloorDeg { get; init; } = 5f;       // off the head ONE tick before = a pull, not a track
    public int SnapMinSnaps { get; init; } = 2;             // repeated pulls (one could be tick-aliasing)
}

// One row per (non-utility) kill: the raw components every candidate deadaim/killWall formula needs.
// One row per enemy bullet hurt (pellets excluded): the silent-aim raw measurement, so the
// off-floor gate can be read off the honest population's tail instead of imported from kill data.
internal readonly record struct HurtRow(
